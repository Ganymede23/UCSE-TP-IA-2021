...
def robot_range(variables, values):
    ...
    has_small_batteries = 'baterias_chicas' in [
        upgrade[0] for upgrade in values]
    has_medium_batteries = 'baterias_medianas' in [
        upgrade[0] for upgrade in values]
    has_big_batteries = 'baterias_grandes' in [
        upgrade[0] for upgrade in values]
...
constraints.append(
    (('comms', 'propulsion'), radio_uncompatible_with_better_engines))
...
constraints.append(
    (('comms', 'propulsion'), videocall_requires_tracks_or_legs))
